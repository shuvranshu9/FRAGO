...
export const getCartByUserId = async (userId) => {
    // First ensure a cart exists for the user
    ...
    // Now fetch items with perfume and variant details
...
export const updateCartItemQuantity = async (userId, cartItemId, quantity) => {
    ...
    if (quantity <= 0) {
        ...
        await pool.query("DELETE FROM cart_item WHERE cart_item_id = ?", [cartItemId]);
    } else {
        await pool.query(
            "UPDATE cart_item SET quantity = ? WHERE cart_item_id = ?",
            [quantity, cartItemId]
...
export const clearCart = async (userId) => {
    // Get cart id
    const [cart] = await pool.query("SELECT cart_id FROM cart WHERE user_id = ?", [userId]);
    if (cart.length === 0) return true;

    await pool.query("DELETE FROM cart_item WHERE cart_id = ?", [cart[0].cart_id]);
    return true;
};
