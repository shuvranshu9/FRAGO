...
const Navbar = () => {
  ...
  return (
    <>
      ...
      >
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-24">
            {/* Left: Search Bar -> Recommendation Trigger (Hidden on mobile, visible on lg) */}
            <div className="hidden lg:flex flex-1 items-center justify-start">
              ...
              >
                <div className="w-full bg-gray-50 border border-gray-200 rounded-full py-2.5 pl-5 pr-12 text-sm text-gray-400 text-left transition-all group-hover:bg-white group-hover:border-green-300 group-hover:shadow-sm">
                  Hey, what are you looking for?
                </div>
                <div className="absolute right-0 top-1/2 -translate-y-1/2 mr-3 p-1.5 text-gray-500 group-hover:text-green-900 transition-colors bg-white/0 group-hover:bg-green-50 rounded-full">
                  <Search size={20} />
                </div>
              </button>
            </div>
            ...
            <div className="flex-1 flex items-center justify-end space-x-6">
              {/* Mobile Search Icon */}
              {/* Mobile Search Icon -> Recommendation Trigger */}
              <button
                onClick={() => setIsRecommendOpen(true)}
                className="lg:hidden p-2 text-gray-600 hover:text-green-900 transition-colors"
              >
                <Search size={22} />
              </button>
              ...
              >
                <div className="relative">
                  ...
                  {unreadMessagesCount > 0 && (
                    <span className="absolute -top-2 -right-0.5 bg-green-600 text-white text-[10px] font-bold h-4 w-4 flex items-center justify-center rounded-full animate-in zoom-in duration-300">
        ...
        >
          ...
          >
            ...
            <div className="p-6 space-y-8 overflow-y-auto h-[calc(100vh-80px)] bg-white">
              {/* Mobile Search */}
              {/* Mobile Search -> Recommendation Trigger */}
              <button
                onClick={() => {
                  setIsOpen(false);
              ...
              >
                <div className="w-full bg-gray-50 border border-gray-200 rounded-lg py-3.5 pl-4 pr-10 text-sm text-gray-400">
                  Tap to find recommendations...
                </div>
                <Search
                  size={18}
              ...
              <div className="space-y-2">
                ...
                {["Perfumes", "Brands", "About-Us", "Contact-Us"].map(
                  (item) => {
                    const path = `/${item.toLowerCase()}`;
                    const isActive = isActiveLink(path);
      ...
      {/* Recommendation Chat Popup - Moved outside nav to fix stacking context/centering issues */}
      {isRecommendOpen && (
        <Recommendation
          key="recommend-popup"
